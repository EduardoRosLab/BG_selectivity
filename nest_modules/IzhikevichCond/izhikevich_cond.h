...
private:
  ...
  struct Buffers_
  {
     ...
     */
    Buffers_( izhikevich_cond& );
    Buffers_( const Buffers_&, izhikevich_cond& );
...
inline port
izhikevich_cond::send_test_event( Node& target, rport receptor_type, synindex, bool )
{
  SpikeEvent e;
  e.set_sender( *this );
...
inline port
izhikevich_cond::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type <= 0
...
inline port
izhikevich_cond::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
izhikevich_cond::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
izhikevich_cond::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d, P_ );
...
inline void
izhikevich_cond::set_status( const DictionaryDatum& d )
{
...
#endif /* #ifndef IZHIKEVICH_COND_H */
