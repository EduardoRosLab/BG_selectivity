...
class STN : public Archiving_Node
{
...
private:
  friend class RecordablesMap< STN >;
  friend class UniversalDataLogger< STN >;
  ...
  struct State_
  {
    ...
    double u_1_; // membrane recovery variable
    double u_2_; // membrane recovery variable
  ...
  struct Buffers_
  {
     ...
     */
    Buffers_( STN& );
    Buffers_( const Buffers_&, STN& );
    UniversalDataLogger< STN > logger_;
  ...
  static RecordablesMap< STN > recordablesMap_;
  /** @} */
};


inline port
STN::send_test_event( Node& target, rport receptor_type, synindex, bool )
{
  SpikeEvent e;
  e.set_sender( *this );
...
inline port
STN::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type <= 0
...
inline port
STN::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
STN::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
STN::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d, P_ );
...
inline void
STN::set_status( const DictionaryDatum& d )
{
