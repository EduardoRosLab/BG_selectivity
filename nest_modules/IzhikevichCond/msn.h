...
/* BeginDocumentation
   ...
   vp        double - Resting membrane potential (v_r in [1])
   ...
   alpha_1    double - beta_1 in [2]
   alpha_2    double - beta_2 in [2]
   alpha      alpha in Humphries Nov 2009
...
class MSN : public Archiving_Node
{
...
private:
  friend class RecordablesMap< MSN >;
  friend class UniversalDataLogger< MSN >;
  ...
  struct Parameters_
  {
    ...
    /** Scaling coefficients for relationship between dopamine receptor occupancy and magnitude **/
    double alpha_1_, alpha_2_;

    /** Size of DA effect in D2 neurons **/
    double alpha_;
  ...
  struct Buffers_
  {
    ...
    UniversalDataLogger< MSN > logger_;
  ...
  //! Read out the AMPA input current
  double
  get_AMPA_current_() const
  {
    return S_.y_[0];
  }
  //! Read out the NMDA input current
  double
  get_NMDA_current_() const
  {
    return S_.y_[1];
  }
  //! Read out the GABA input current
  double
  get_GABA_current_() const
  {
    return S_.y_[2];
  }
  // ----------------------------------------------------------------
  ...
  static RecordablesMap< MSN > recordablesMap_;
  /** @} */
};


inline port
MSN::send_test_event( Node& target, rport receptor_type, synindex, bool )
{
  SpikeEvent e;
  e.set_sender( *this );
...
inline port
MSN::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type <= 0
...
inline port
MSN::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
MSN::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
MSN::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d, P_ );
...
inline void
MSN::set_status( const DictionaryDatum& d )
{
